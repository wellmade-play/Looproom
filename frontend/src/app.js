...
const DEFAULT_API_BASE = (() => {
    const envBase = sanitizeBase(import.meta.env.VITE_API_BASE);
    if (envBase)
        return envBase;
...
const elements = {
    appShell: select('#app'),
    offlineNotice: document.querySelector('#offlineNotice') ?? document.body,
    artistList: select('#artistList'),
    roomList: select('#roomList'),
    roomTitle: select('#roomTitle'),
    roomSubtitle: select('#roomSubtitle'),
    joinLeaveBtn: select('#joinLeaveBtn'),
    chatTimeline: select('#chatTimeline'),
    chatForm: select('#chatForm'),
    chatInput: select('#chatInput'),
    sendMessageBtn: select('#sendMessageBtn'),
    refreshMessagesBtn: select('#refreshMessagesBtn'),
    spotifyPlayer: select('#spotifyPlayer'),
    nowPlayingTitle: select('#trackTitle'),
    nowPlayingArtist: select('#trackArtist'),
    nowPlayingProgress: select('#trackProgress'),
    nowPlayingDuration: select('#trackDuration'),
    nowPlayingArtwork: select('#trackArtwork'),
    refreshPlaybackBtn: select('#updatePlaybackBtn'),
    queueList: select('#queueList'),
    refreshQueueBtn: select('#refreshQueueBtn'),
    openQueueDialogBtn: select('#openQueueDialog'),
    recommendationList: select('#recommendationList'),
    refreshRecommendationsBtn: select('#refreshRecommendationsBtn'),
    focusModeToggle: select('#focusModeToggle'),
    roomSearch: select('#roomSearch'),
    requestDialog: select('#requestDialog'),
    requestForm: select('#requestForm'),
    requestTrackUri: select('#requestTrackUri'),
    requestNote: select('#requestNote'),
};
const fallbackUser = {
    id: 'offline-user',
    spotify_id: 'offline',
...
const updateSpotifyEmbed = (track) => {
    if (!track) {
    ...
    const fallbackLink = (() => {
        ...
        if (track.uri?.startsWith('https://open.spotify.com/track/'))
            return track.uri;
        return null;
    })();
    if (fallbackLink) {
...
const renderArtistList = (artists) => {
    ...
    for (const artist of artists) {
        ...
        button.type = 'button';
        if (artist.id === state.selectedArtistId)
...
const renderRoomDetailsPlaceholder = () => {
...
const renderRecommendations = (roomId) => {
...
const ensureUser = async () => {
    try {
        ...
        if (status === 401) {
            try {
                const login = await fetchJSON(`${ROOT_BASE}/auth/spotify/login?redirect_uri=${encodeURIComponent(window.location.href)}`);
...
const loadArtists = async () => {
    try {
        const artists = await fetchJSON('/artists');
        state.offline = false;
        elements.offlineNotice.classList.add('hidden');
...
const refreshMessages = async (roomId) => {
    try {
        const messages = await fetchJSON(`/rooms/${roomId}/messages?limit=100`);
...
const refreshQueue = async (roomId) => {
    try {
        const queue = await fetchJSON(`/rooms/${roomId}/queue`);
...
const refreshRecommendations = async (roomId) => {
    try {
        const data = await fetchJSON(`/rooms/${roomId}/recommendations`);
...
const handleChatSubmit = async (event) => {
    ...
    try {
        let message;
        if (!state.offline) {
            message = await fetchJSON(`/rooms/${roomId}/messages`, {
                method: 'POST',
...
const handleQueueRequest = async (event) => {
    event.preventDefault();
    const roomId = state.currentRoomId;
    if (!roomId || !state.user)
        return;
...
const setupEventListeners = () => {
    ...
    elements.refreshMessagesBtn.addEventListener('click', () => {
        if (state.currentRoomId)
            void refreshMessages(state.currentRoomId);
    });
    ...
    elements.refreshQueueBtn.addEventListener('click', () => {
        if (state.currentRoomId)
            void refreshQueue(state.currentRoomId);
    });
    elements.refreshRecommendationsBtn.addEventListener('click', () => {
        if (state.currentRoomId)
            void refreshRecommendations(state.currentRoomId);
    });
